		...
		methods: {
			...
			update: function (json, confirm) {
				if (json.response.redirect) {
					...
					if ($('.select-parcel').length){
	                    $('.select-parcel').attr('style', 'border: 1px solid #ccc !important;');
	                }

					if (json.response.error) {
						$('#quick-checkout-button-confirm').button('reset');

	                    // Custom validation for pickup points
	                    if (json.response.error.dpd_parcel) {
	                        if ($('.select-parcel').length){
	                            $('.select-parcel').attr('style', 'border: 1px solid rgba(248, 86, 57, 1) !important;');
	                            $('.select-parcel').click( function(){
	                                $('.select-parcel').attr('style', 'border: 1px solid #ccc');
	                            });
	                            $('html, body').animate({ scrollTop: $('.select-parcel').offset().top - 150 }, 'slow');
	                        } else {
	                            $('html, body').animate({ scrollTop: $('#dpd-selected-parcel').offset().top - 150 }, 'slow');
	                        }
