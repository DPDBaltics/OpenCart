$(function () {
	window['_QuickCheckout'] = new Vue({
		...
		updated: function () {
			...
			$('#shipping_method_form').find('.select-parcel').on('change', function (event) {
		    	//var selector = $(this).parent().parent().find('input[name="shipping_method"]'); 
		    	vId = $.trim($(this).parent().parent().find('input[name="shipping_method"]').attr('id').replace('dpd_parcel.dpd_parcel_',''));
		    	$.ajax({
		    ...
		    $('#shipping_method_form').find('.timeframe').on('change', function (event) {
		    	//var selector = $(this).parent().parent().find('input[name="shipping_method"]'); 
		    	cId = $.trim($(this).attr('id').replace('dpd_courier_',''));		    	
		    	$.ajax({
			      url: 'index.php?route=checkout/checkout/dpd_set_terminal_courier',
			      dataType: 'json',
			     // method: 'POST',
			      data: {
			      	code: $(this).val()
			      ...
			      complete: function() {	      	
					if(cId){
						$("#shipping_method_form").find('input:radio').prop('checked',false);	
						$("#shipping_method_form").find('input:radio').each(function() {
							if($.trim($( this ).attr( 'id' )) == 'dpd_courier.dpd_courier_'+cId){
								$( this ).click();
							}
						});
					}			      	    	
